...
contract SwapMarketV1 is Ownable, IERC721Receiver, ThirdwebContract{
  ...
  modifier onlyInitiator(uint256 swapId) {
    require(msg.sender == _swaps[swapId].initiator,
      "SwapMarket: caller is not swap initiator");
    _;
  }

  modifier requireSameLength(address[] memory nftAddresses, uint256[] memory nftIds) {
    require(nftAddresses.length == nftIds.length, "SwapMarket: NFT and ID arrays have to be same length");
    _;
  }

  modifier chargeAppFee() {
    require(msg.value >= fee, "SwapMarket: Sent ETH amount needs to be more or equal application fee");
    _;
  }
  ...
  function initiateSwap(uint256 swapId, address[] memory nftAddresses, uint256[] memory nftIds)
    external payable chargeAppFee requireSameLength(nftAddresses, nftIds) {
      require(_swaps[swapId].secondUser == msg.sender, "SwapMarket: caller is not swap participator");
      require(
        ...
        "SwapMarket: swap already initiated"
      );
  ...
  function acceptSwap(uint256 swapId) external onlyInitiator(swapId) {
    require(
      ...
      (_swaps[swapId].initiatorNftAddresses.length != 0 || _swaps[swapId].initiatorEtherValue > 0),
       "SwapMarket: Can't accept swap, both participants didn't add NFTs"
    );
  ...
  function cancelSwap(uint256 swapId) external {
    require(
      ...
      "SwapMarket: Can't cancel swap, must be swap participant"
    );
  ...
  function withdrawEther(address payable recipient) external onlyOwner {
    require(recipient != address(0), "SwapMarket: transfer to the zero address");
